...
export const CartSidebar: React.FC<CartSidebarProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-50 overflow-hidden animate-in fade-in duration-200">
      ...
      {/* Drawer - Perfectly straight, flush to right edge */}
      <div className="fixed inset-y-0 right-0 w-full max-w-md bg-white dark:bg-[#1a1a1a] shadow-2xl flex flex-col justify-between border-l border-gray-200 dark:border-gray-800 z-50 animate-in slide-in-from-right duration-300">
        {/* Header */}
        <div className="bg-[#f68b1e] text-white px-5 py-4 flex items-center justify-between shadow-xs shrink-0">
          <div className="flex items-center gap-2 font-bold text-base tracking-wide">
            <ShoppingCart className="w-5 h-5" />
            <span>My Shopping Cart ({totalCount})</span>
          </div>
          <button
            onClick={onClose}
            className="p-1.5 rounded-full hover:bg-black/15 text-white transition-colors cursor-pointer"
                  ...
                  <div className="flex-1 min-w-0 space-y-1">
                    ...
                    <div className="flex items-center gap-2 pt-1">
                      <div className="flex items-center border border-gray-200 dark:border-gray-700 rounded-lg overflow-hidden bg-gray-50 dark:bg-[#242424]">
                        ...
                        <span className="px-2 text-xs font-bold text-gray-800 dark:text-gray-200 min-w-5 text-center">
                          {item.qty}
                        </span>
                        <button
                          onClick={() => onChangeQty(item.id, 1)}
