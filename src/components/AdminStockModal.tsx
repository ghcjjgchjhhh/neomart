  ...
  return (
    <div className="fixed inset-0 z-50 overflow-y-auto bg-black/75 backdrop-blur-sm flex items-center justify-center p-3 sm:p-4">
      <div className="bg-white dark:bg-[#18181b] border border-gray-200 dark:border-gray-800 rounded-3xl max-w-5xl w-full shadow-2xl overflow-hidden flex flex-col max-h-[92vh]">
        ...
        {showEditor && editing && <form onSubmit={saveProduct} className="border-b border-gray-200 bg-orange-50 p-4 dark:border-gray-800 dark:bg-orange-950/20"><div className="mb-3 flex items-center justify-between"><h4 className="font-extrabold">{products.some((product) => product.id === editing.id) ? 'Edit product' : 'Add product'}</h4><button type="button" onClick={() => setShowEditor(false)} aria-label="Close product editor"><X className="h-4 w-4" /></button></div><div className="grid grid-cols-1 gap-2 sm:grid-cols-2 lg:grid-cols-3">{([['name','Product name'],['price','Price'],['oldPrice','Sale/original price'],['discount','Discount %'],['category','Category'],['images','Image URLs (comma separated)']] as const).map(([key,label]) => <label key={key} className="text-[11px] font-bold">{label}<input required={key === 'name'} value={key === 'images' ? (editing.images || []).join(', ') : String(editing[key] ?? '')} onChange={(event) => setEditing({ ...editing, [key]: key === 'images' ? event.target.value.split(',').map((url) => url.trim()) : event.target.value } as Product)} type={['price','oldPrice','discount'].includes(key) ? 'number' : 'text'} className="mt-1 w-full rounded-lg border border-gray-300 bg-white px-3 py-2 text-xs font-normal dark:border-gray-700 dark:bg-[#202024]" /></label>)}</div><div className="mt-3 flex flex-wrap items-center gap-3 text-xs"><label className="flex items-center gap-2"><input type="checkbox" checked={Boolean(editing.featured)} onChange={(event) => setEditing({ ...editing, featured: event.target.checked })} /> Featured product</label><label className="flex items-center gap-2">Low-stock warning below <input type="number" min="0" value={editing.lowStockThreshold ?? 5} onChange={(event) => setEditing({ ...editing, lowStockThreshold: Number(event.target.value) })} className="w-16 rounded border px-2 py-1" /></label><button type="submit" className="rounded-lg bg-[#f68b1e] px-4 py-2 font-bold text-white">Save product</button></div></form>}
