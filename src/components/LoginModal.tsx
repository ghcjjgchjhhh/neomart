...
import { ArrowLeft, Bell, Check, ChevronRight, CreditCard, Eye, EyeOff, FileText, Headphones, HelpCircle, Info, LoaderCircle, LockKeyhole, LogOut, Mail, MapPin, Package, Pencil, RotateCcw, ShieldCheck, ShoppingCart, Smartphone, Tag, Truck, UserRound, X } from 'lucide-react';
      ...
      <div className={`fixed inset-0 z-[70] flex min-h-dvh items-center justify-center overflow-y-auto px-5 py-8 sm:px-8 ${authBackdropClass}`}>
        <div className={`relative w-full max-w-[470px] rounded-[22px] border px-5 py-8 shadow-2xl sm:px-10 sm:py-10 ${authPanelClass}`}>
          ...
          <form onSubmit={handleEmailOnlyGoogleSignIn} className="mt-7">
            ...
            <button type="submit" disabled={loading} className={`mt-5 flex h-14 w-full items-center justify-center gap-3 rounded-[10px] border bg-transparent text-[14px] font-bold transition disabled:cursor-not-allowed disabled:opacity-60 ${authSecondaryButtonClass}`}>{loading ? <LoaderCircle className="h-6 w-6 animate-spin" /> : <GoogleIcon />}<span>{loading ? 'Signing in...' : 'Continue with email address'}</span></button>
          </form>
